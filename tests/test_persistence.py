...
async def test_volume_persistence(async_db_pool, event_factory):
    ...
    dsn = os.getenv(
        "TEST_POSTGRES_DSN",
        os.getenv("POSTGRES_DSN", "postgresql://pubsub:pubsub123@localhost:5432/pubsub"),
    )
...
async def test_persistence_setelah_restart(event_factory):
    ...
    assert second_result is False, "Setelah restart, event yang sama harus tetap dianggap duplikat"
