...
@pytest.mark.asyncio
async def test_concurrent_same_event(async_db_pool, event_factory):
	"""Sepuluh coroutine yang mengirim event sama harus menghasilkan satu baris saja."""
	event = event_factory()
	results = await asyncio.gather(*(process_event(event, async_db_pool) for _ in range(10)))
	assert results.count(True) == 1, "Hanya satu coroutine yang boleh memproses event sebagai baru"
	assert results.count(False) == 9, "Sembilan coroutine lain harus melihat event sebagai duplikat"
	...
	assert count == 1, f"Harus ada satu baris saja di DB, ditemukan {count}"


@pytest.mark.asyncio
async def test_no_race_condition(async_db_pool, event_factory):
	"""Beban besar dengan duplikat harus tetap konsisten antara tabel dan statistik."""
	base_events = [event_factory(topic="payment.processed") for _ in range(800)]
	duplicate_source = base_events[:200]
	all_events = base_events + duplicate_source
	results = await asyncio.gather(*(process_event(event, async_db_pool) for event in all_events))
	async with async_db_pool.acquire() as connection:
		stats = await connection.fetchrow("SELECT received, unique_processed, duplicate_dropped FROM stats WHERE id = 1")
		count = await connection.fetchval("SELECT COUNT(*) FROM processed_events")
	assert stats["received"] == len(all_events), "received harus sama dengan total event yang diproses"
	assert stats["unique_processed"] == count, "unique_processed harus sama dengan jumlah baris unik di DB"
	assert stats["received"] == stats["unique_processed"] + stats["duplicate_dropped"], "Rumus statistik harus seimbang"


@pytest.mark.asyncio
async def test_stats_atomic(async_db_pool, event_factory):
	"""Statistik harus selalu konsisten setelah beban paralel."""
	events = [event_factory(topic="system.error") for _ in range(50)]
	await asyncio.gather(*(process_event(event, async_db_pool) for event in events))
	async with async_db_pool.acquire() as connection:
		stats = await connection.fetchrow("SELECT received, unique_processed, duplicate_dropped FROM stats WHERE id = 1")
	assert stats["received"] == stats["unique_processed"] + stats["duplicate_dropped"], "Statistik harus atomik dan konsisten"


@pytest.mark.asyncio
async def test_multi_worker_no_double_process(async_db_pool, event_factory):
	"""Empat worker yang membaca job sama tidak boleh memproses event ganda."""
	events = [event_factory(topic="sensor.reading") for _ in range(40)]
	buffer = []
	for event in events:
		buffer.append(event)
		buffer.append(event)

	results = await asyncio.gather(*(process_event(event, async_db_pool) for event in buffer))
	async with async_db_pool.acquire() as connection:
		count = await connection.fetchval("SELECT COUNT(*) FROM processed_events WHERE topic = $1", "sensor.reading")
	assert count == 40, f"Hanya 40 event unik yang boleh tersimpan, tetapi ditemukan {count}"
	assert results.count(True) == 40, "Setiap event unik harus diproses sekali"from __future__ import annotations
...
async def test_concurrent_same_event(async_db_pool: asyncpg.Pool, event_factory):
	...
	results = await asyncio.gather(
		*(process_event(event, async_db_pool) for _ in range(10))
	)
...
async def test_no_race_condition(async_db_pool: asyncpg.Pool, event_factory):
	...
	duplicate_events = [event_factory(topic="sensor.reading", event_id=unique_events[index].event_id) for index in range(200)]
...
async def test_multi_worker_no_double_process(async_db_pool: asyncpg.Pool, redis_client, event_factory):
	...
	for _ in range(count_total):
		await redis_client.xadd(stream_name, {"event": event_factory(topic="system.error").model_dump_json()})
		...
		while True:
			...
			for _, messages in response:
				for message_id, fields in messages:
					await process_event(event_factory(topic="system.error", event_id=fields["event"][0:36]), async_db_pool)
