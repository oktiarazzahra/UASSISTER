...
async def test_stats_initial(test_client):
    ...
    assert data["topics"] == [], "topics awal harus kosong"
...
@pytest.mark.asyncio
async def test_stats_update_after_processing(test_client, async_db_pool: asyncpg.Pool, event_factory):
    """Stats harus benar setelah event diproses lewat process_event dan publish."""
    events = [event_factory(topic="order.created") for _ in range(7)]
...
async def test_health_endpoint(test_client):
    ...
    assert response.status_code == 200, f"Status kode harus 200, bukan {response.status_code}"
    body = response.json()
    assert body["status"] == "ok", "status health harus ok"
    assert body["db"] == "connected", "DB harus connected"
    assert body["broker"] == "connected", "Broker harus connected"
